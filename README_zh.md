...
- 可选 `VLC` 后端，通过 HTTP 接口控制
- 仅托盘应用控制，包含动态的开始/停止 Cast 标签
- 本地设置服务器，带可编辑表单，持久化保存配置
...
1. `开始 Cast` / `停止 Cast`（复选框，反映当前 Cast 状态）
2. `更多` >（子菜单）
   - `打开设置`
   - `打开目录`（在文件管理器中打开配置/日志目录）
3. `退出程序`
...
应用启动时自动开始 Cast 服务并启动设置 HTTP 服务器。设置服务器绑定到
`127.0.0.1:7878`（端口冲突时选择临时端口），点击 `打开设置` 时使用系统默认
浏览器打开页面。
...
```text
应用启动           -> 绑定 127.0.0.1:7878
端口冲突           -> 选择临时端口作为备选
打开设置点击       -> 在默认浏览器中打开页面
```

服务器仅监听本地回环地址。当前设置包括设备名称、播放器后端、mpv 路径和
vlc 路径。
...
当前接口涵盖加载、播放、暂停、停止、跳转、音量、静音、状态查询和协议信息
报告。未来的后端（如 GStreamer 或 FFmpeg）应实现此 trait 并在
`player::create_backend` 中注册。
...
优先后续工作：并发请求处理、持久化每安装的 UPnP UDN、DIDLLite 元数据和
集成测试。
